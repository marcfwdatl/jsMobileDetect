var MobileDetect = MD = (function( window, undefined )
{
  ...
  var uagent = "",
    ...
    phoneDevices = {
        ...
        'Motorola'     : 'Motorola|\bDroid\b.*Build|DROIDX|Android.*Xoom|HRI39|MOT-|A1260|A1680|A555|A853|A855|A953|A955|A956|Motorola.*ELECTRIFY|Motorola.*i1|i867|i940|MB200|MB300|MB501|MB502|MB508|MB511|MB520|MB525|MB526|MB611|MB612|MB632|MB810|MB855|MB860|MB861|MB865|MB870|ME501|ME502|ME511|ME525|ME600|ME632|ME722|ME811|ME860|ME863|ME865|MT620|MT710|MT716|MT720|MT810|MT870|MT917|Motorola.*TITANIUM|WX435|WX445|XT300|XT301|XT311|XT316|XT317|XT319|XT320|XT390|XT502|XT530|XT531|XT532|XT535|XT603|XT610|XT611|XT615|XT681|XT701|XT702|XT711|XT720|XT800|XT806|XT860|XT862|XT875|XT882|XT883|XT894|XT909|XT910|XT912|XT928|RAZR',
    ...
    operatingSystems = {
        ...
        'iOS'              : 'iphone|ipod|ipad|iPhone|iPod|iPad|CPU OS 5_|CPU OS 6_',
        'FlashLiteOS'      : '',
  ...
  var getBrowser = function()
  {
    for( var i in userAgents ) {
      if( uagent.search( userAgents[i] ) > -1 ) return i; 
    }
  ...
  var getOS = function()
  {
    for( var i in operatingSystems ) {
      if( uagent.search( operatingSystems[i] ) > -1 ) return i; 
    }
    
    return 'not-found';
  }
  
  var getMobileDeviceName = function()
  {
    for( var i in phoneDevices ) {
      if( uagent.search( phoneDevices[i] ) > -1 ) return i; 
    }
    
    return 'not-found';
  }
  
  var getTabletDeviceName = function()
  {
    for( var i in tabletDevices ) {
      if( uagent.search( tabletDevices[i] ) > -1 ) return i; 
    }
  ...
  {
    return (uagent.search( phoneDevices['iPhone'] ) > -1);
  }
  
  var isIPad = function()
  {
    return (uagent.search( tabletDevices['iPad'] ) > -1);
  }
  
  var isAndroid = function()
  {
    return (uagent.search( operatingSystems['AndroidOS'] ) > -1);
  }
  
  var isAndroidMobile = function()
  {
    return (uagent.search( userAgents[ 'AndroidMobile' ] ) > -1);
  }
  
  var isIOS = function()
  {
    return (uagent.search( operatingSystems['iOS'] ) > -1);
  }
  
  var isMobileDevice = function()
  {
    for( var i in phoneDevices ) {
      if( uagent.search( phoneDevices[i] ) > -1 ) return true; 
    }
  ...
  {
    for( var i in tabletDevices ) {
      if( uagent.search( tabletDevices[i] ) > -1 ) return true; 
    }
  ...
  {
    return (uagent.search( userAgents['Chrome'] ) > -1 );
  } 
  
  var isBrowserSafari = function()
  {
    return (uagent.search( userAgents['Safari'] ) > -1 );
  } 
  
  var isAppleProduct = function()
  {
    return (isIOS() && (isIPad() || isIPhone()));
  }
  ...
  */
  return {
    ...
    {
      return initInnerWidth;
    },
    getInnerHeight : function()
    {
      return initInnerHeight;
    },
    getBrowser : getBrowser,
    getOS : getOS,
    getMobileDeviceName : getMobileDeviceName,
    getTabletDeviceName : getTabletDeviceName,
    isDevice : function()
    {
      return ( isMobileDevice() || isTabletDevice() || isAndroid() || isIOS() );
    },
    isIPhone : isIPhone,
    isIPad : isIPad,
    isIOS : isIOS,
    ...
    {
      return ( isMobileDevice() || isAndroidMobile() );
    },
    isTabletDevice : isTabletDevice,
    isBrowserChrome : isBrowserChrome,
    isBrowserSafari : isBrowserSafari,
    isIDevice : isAppleProduct,
    isAppleProduct : isAppleProduct,
    
    // public methods //
    init: function()
    {
      initInnerHeight = window.innerHeight;
      initInnerWidth = window.innerWidth;
      if (navigator && navigator.userAgent) uagent = navigator.userAgent;//"Mozilla/5.0 (Linux; U; Android 4.0.4; en-us; DROID RAZR 4G Build/6.7.2-180_DHD-16_M4-31) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 Mobile Safari/534.40"; 
    }
  }
